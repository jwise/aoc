...
function run(arr,grid)
	...
	st = { mem = memdup(arr), ifif = ifif, ofif = ofif, pc = 0, done = false, blocked = false }
	while not st.done do
		...
		if #ofif == 2 then
			...
			if not grid[y] then grid[y] = {} end
			table.insert(ifif, grid[y][x] or 0)
		end
		if st.blocked and not st.done then abort() end
	end
end
