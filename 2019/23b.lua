...
for i=0,49 do
	...
	local ofif = {}
	inpqs[i] = {}
	local st = { mem = memdup(arr), ifif = ifif, ofif = ofif, inpq = inpqs[i], addr = i, pc = 0, done = false, blocked = false, rxctr = 0 }
	table.insert(sts, st)
end
...
while true do
	for n,st in ipairs(sts) do
		...
		if st.blocked and #st.ifif == 0 then
			...
			st.rxctr = st.rxctr + 1
			if st.rxctr > RXMAX then st.rxctr = RXMAX end -- clamp
		end
	...
	local isidle = true
	for n,st in ipairs(sts) do
		if st.rxctr ~= RXMAX then
			isidle = false
		end
	end
	if isidle and nat then
		print("NAT releasing: X " .. nat.x .. ", Y " .. nat.y)
		table.insert(sts[1].inpq, nat)
		nat = nil
	end
end
